...
## Reconciliation of 2026-08-13

Rows below were written against the pre-restoration starter world. They are
reconciled here against the recorded operator supersessions in
`docs/FAILURE_LOG.md` — "Batch operator decision: restored Garden destination
after return review" (2026-08-11), "Wayfinder refinement: close product E2E and
Garden as independent acceptance lanes" (2026-08-11), and "Gate 2/12 blocker
wording reconciled without gate promotion" (2026-08-13). Three demands are
withdrawn or superseded and are marked inline: terminal traversal as a release
gate, animal/collectible actions, and broad author Garden placement controls.
No gate is promoted; every previously open obligation stays open.

The accepted default composition is one seed-selected flower at world center
inside the six accepted base fixtures, with no animals, no collectibles and no
autonomous animation (`STARTER_ANIMAL_SPECIES` and `STARTER_COLLECTIBLES` are
empty in `src/lateletter/garden/world/generation.py`). Journal/inventory access
is mailbox-owned and conditional on canonical content. Recipient traversal of
that composition is EXECUTED on `cfabe67` (2026-08-12 product E2E successor
receipts) and `bdab368` (2026-08-13 gesture-ownership repair). VoiceOver/NVDA
are excluded by operator direction, so this document claims no screen-reader
...
| Touch / mouse / keyboard / terminal input | All canonical commands have discoverable terminal keys/actions | Native controls derive touch/mouse; keyboard bindings route to the same command bytes | Yes at adapter/reducer level | Gate 2 PARTIAL, reconciled 2026-08-13. Terminal traversal is WITHDRAWN (2026-08-11) and is no longer a Gate 2 demand. The demand is now every action the accepted composition exposes; desktop mouse/keyboard and native touch at 390 and 320 CSS pixels are EXECUTED on `cfabe67` and `bdab368`. Physical-handset touch and no-color use remain OPEN, as does operator interaction acceptance |
...
| Placement, movement, rotation, and undo | Camera-centered copy/place, move, rotate, transplant, undo | Reader chooses kind, catalog, x/y; move/rotate/undo use canonical validation | Yes | Reconciled 2026-08-13. Author Garden placement/move/rotate/undo are SUPERSEDED (2026-08-11) as outside the accepted MVP, so they are not Gate 2 observation demands; the canonical validation, undo bounds and cross-runtime bytes stay covered by the cited reducer/adapter tests. Recipient camera pan, `Home` and empty-ground double-click/double-tap recenter are EXECUTED on `bdab368` |
...
| Collectibles, inventory, and journal | Persistent collect/examine/inventory/journal and authored memories; scrollable inventory/journal/missed-summary panel | Same state and complete journal; browser object-list/action-card surface withdrawn after visual rejection | Partial, and outside the accepted default | Reconciled 2026-08-13. Collect/examine are SUPERSEDED (2026-08-11) as acceptance demands because the accepted default composition contains no collectible; adding one is a later destination change. Journal/inventory is mailbox-owned and conditional on canonical content, reached by keyboard and native touch on `cfabe67` and `bdab368`. Screen-reader replacement stays UNCLAIMED — VoiceOver/NVDA are excluded by operator direction, which is an exclusion, not evidence. Rejected labels must not return |
| Deterministic animal AI | Four species; safety-first priority, personality/needs, bounded episodic memory, weather/season utility, deterministic validated locomotion/routines, hysteresis, four bond tiers, authored choreography | Same decisions/movement/state; reason, score, context, tier, intent, personality, and memory are visible and semantically described | Yes—decision, position, projection, and restart bytes match | Gate 7 PASS on authored or populated worlds. Noted 2026-08-13: the accepted default composition places no animal, so feed/play and animal presentation are not part of the accepted scenario (2026-08-11); this row claims engine parity, not accepted-default behaviour |
...
| Rich presentation reconstruction after `520f27b` regression | Portable atlas glyph projection remains deliberately compact; the July curated starter world described here contained 10 fixtures, 8 plants, 4 animals, and 3 collectibles, which the 2026-08-11 restored destination SUPERSEDES with six accepted base fixtures plus one seed-selected flower and no animals or collectibles | Responsive 4–22-row read-only compositor with bounded LOD, deterministic independent-unit no-overlap packing, per-cell plant motion, complete animal intent families and four visible projected bond tiers, distinct canonical-species silhouettes at every density, semantic collectibles, continuous cover, civil-time/season palettes, dense storybook night sky, differentiated ambient life, semantic-surface weather reactions, exact-ID focus/click/feed effects, and memorial art without an unprojected animal | Yes—presentation consumes the same projection and never owns gameplay/target/persistence state | Machine-addressable completion-audit corrections hold locally; the animal, collectible and autonomous-motion presentation paths listed here are not exercised by the accepted default (2026-08-11). Operator visual acceptance, physical-handset touch, and the supported browser screenshot matrix remain OPEN |
| Versioned Unicode/ASCII atlas | Python consumes and validates the canonical atlas manifest, semantic tokens, graphemes, and safe profiles | Browser imports that same manifest directly | Yes—no duplicate glyph owner | Gate 6 PARTIAL: the supported browser screenshot matrix remains OPEN. Its terminal half is WITHDRAWN with terminal parity (2026-08-11), narrowing the obligation without satisfying it |
...
| Accessibility | Line-readable state, ordinary-text help, journal/inventory/absence/memorial | Bounded Garden summary/inventory label, semantic per-object descriptions, named action/status region, focusable controls, 44px targets, reduced motion, and theme/HUD normal-text tokens at ≥4.5:1 computed contrast | Same projected facts/actions | Gate 12 PARTIAL, reconciled 2026-08-13: 390×844 and fresh 1280×720 browser layouts cleared geometry/overflow/target diagnostics, and 200% was EXECUTED via CDP page scale 2 at the authoritative desktop layout on `cfabe67`. VoiceOver/NVDA are EXCLUDED by operator direction (2026-08-11), so no screen-reader result is claimed. No-color use, physical-handset touch, and direct human observation remain OPEN |
...
| **PARTIAL** | 1 Published production reachability; 2 input traversal of the accepted composition (terminal half withdrawn 2026-08-11; physical-handset touch and no-color OPEN); 6 atlas screenshot portability (browser half OPEN); 10 reference-device performance; 12 accessibility observation (VoiceOver/NVDA excluded by operator direction; no-color, physical-handset touch and human runs OPEN) |
...
Terminal traversal was WITHDRAWN as a release-gate demand on 2026-08-11, so the
terminal columns record engine and renderer conformance evidence rather than an
outstanding acceptance obligation; the browser is the accepted product surface.
