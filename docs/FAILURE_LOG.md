...
- Required fix: Give every fixture and every collectible a distinct picture large enough to be recognised, port the archived art where an accepted version already exists, add a contract that every catalog identity maps to a picture unique within its kind, and make level-of-detail reduction select a purpose-drawn smaller picture rather than deleting lines from a larger one.
