...
- Resolution: Added `.github/workflows/block-ai-attribution.yml`, which audits commit author, committer, attribution trailers, and AI session metadata on every push and pull request. Added an active `Block AI attribution` ruleset requiring its `AI attribution policy` status on the default branch. Installed and verified this combination across all 13 repositories that were public at intake and on the replacement private LateLetter repository.
- Status: Fixed with supported workflow-plus-ruleset enforcement
