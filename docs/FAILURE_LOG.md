...
### A queued Pages deploy was misattributed to a billing block instead of the environment branch policy

- Symptom: `gh workflow run "Deploy to GitHub Pages" --ref restore/pre-jul19-viewer` produced run `30361548276`, which sat in `queued` for over 25 minutes and was reported to the operator as a GitHub Actions billing block.
- Actual cause: the `github-pages` environment carries a `branch_policy` protection rule whose sole allowed branch is `main` (`GET /repos/rikiyanai/lateletter/environments/github-pages/deployment-branch-policies` returns `total_count: 1`, `name: main`, `type: branch`). The run's `head_branch` is `restore/pre-jul19-viewer`, so it can never be assigned a runner. The tell is that the run has **zero jobs created** — a branch-policy rejection, not a queue wait. `pending_deployments` was empty, so it was not an approval gate either.
- Root cause of the misreport: the claim was carried over from the older entry "Every push published the site, and the branch policy check could not run without payment" without re-verifying against the live API. A prior failure-log statement was treated as current fact. The operator corrected it, confirming budget remained available.
- Distinct from the operator's intended gate: the workflow is `workflow_dispatch`-only by design, so publishing already requires an explicit manual step. The branch policy is a second, independent restriction and was not part of that design intent.
- Resolution: `restore/pre-jul19-viewer` was added to the `github-pages` deployment branch policy at operator instruction, so the frozen `legacy/` snapshot can publish without first merging to `main`. This widens what can reach production from one branch to two and should be reverted to `main`-only once the legacy snapshot is merged or retired.
- Lesson: environment protection state is live repository configuration and must be read from the API at the time of the failure. A failure-log entry records what was true when written, not what is true now.
- Status: Implemented (unproven) — the policy is changed and the deploy re-dispatched; operator confirmation that the published site serves the legacy snapshot remains open.
