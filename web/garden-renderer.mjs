 ...
 */
function hitRectToHotspot(rect) {
  return {
    x: rect.left,
    y: rect.top,
...
export class CanonicalGardenRenderer {
  ...
  _renderMeasuredAssets(assets) {
    this.measuredAssetRects = new Map();
    if (this.geometry.affineOnly || !assets.length) {
      if (this.measuredLayer) this.measuredLayer.innerHTML = '';
      return;
    }
    if (!this.measuredLayer) {
      this.measuredLayer = document.createElement('div');
      this.measuredLayer.className = 'garden-measured-layer';
      this.measuredLayer.setAttribute('aria-hidden', 'true');
      this.element.appendChild(this.measuredLayer);
    } else if (this.measuredLayer.parentNode === this.element) {
      // Keep the overlay after the lattice rows when a resize adds rows.
      this.element.appendChild(this.measuredLayer);
    }
    const html = [];
    for (const asset of assets) {
      const placed = measuredAssetPlacement(this.geometry, asset);
      this.measuredAssetRects.set(placed.objectId, {
        x: placed.left, y: placed.top, width: placed.width, height: placed.height,
      });
      for (const glyph of placed.glyphs) {
        html.push(`<span style="left:${glyph.x.toFixed(2)}px;top:${glyph.y.toFixed(2)}px` +
          (glyph.color ? `;color:${glyph.color}` : '') +
          `">${escapeHtml(glyph.glyph)}</span>`);
      }
    }
    this.measuredLayer.innerHTML = html.join('');
  }
  measure() {
