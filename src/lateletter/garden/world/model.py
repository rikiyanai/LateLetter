...
# Three versions, deliberately independent of each other.
#
# `WORLD_SCHEMA_VERSION` above is the SHAPE of the serialised document: which
# keys exist and what types they hold. It answers "can this be parsed at all".
#
# `GENERATOR_VERSION` is the version of the CODE that produced a world's
# initial content -- which species, how many, and where they were placed. It
# answers "was this world built by today's generator".
#
# `COMPOSITION_VERSION` identifies which CANDIDATE composition recipe the
# population came from. It is a revision number and nothing more.
#
# It does NOT mean the composition was reviewed, and it must never be read that
# way. An earlier draft of this comment defined it in terms of operator
# approval, and that single sentence made every generated world claim a verdict
# that has never been granted -- the failure log records that no composition has
# been accepted. Acceptance is a separate, fingerprint-bound operator verdict in
# `docs/garden-composition-acceptance.json`; a version number cannot carry a
# verdict, because the number is written by the code and the verdict is given by
# a person.
#
# Why the three cannot be one number: a stored world can be perfectly parseable
# (schema current), built by an old generator, and hold a composition revision
# nobody has looked at -- all three at once. Held in one field, an obsolete
...
class WorldState:
    ...
    # None means "this world predates version stamping", which is a different
    # claim from "this world was built by generator 1" and must never be
    # collapsed into it. A world written before these fields existed cannot
    # honestly say what made it, and saying so is the point.
    generator_version: int | None = None
    composition_version: int | None = None
    # The roster the generator actually produced, recorded at the moment it
    # produced it. A version number alone is an unverified assertion: a world
    # can carry the current number over an arbitrary population and nothing
    # would notice. Recomputing this from the world's contents and comparing is
    # what turns the stamp into evidence, and it is why a custom roster or an
    # author-program-modified world stops matching its own stamp.
    composition_fingerprint: str | None = None
    # Set when a stored world's SHAPE was upgraded by a migration. The content
    # is not regenerated by a migration, so the generator and composition
    # stamps above are never rewritten -- an old world stays an old world.
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> WorldState:
        ...
        return cls(
            ...
            composition_fingerprint=(
                None
                if data.get("composition_fingerprint") is None
                else str(data["composition_fingerprint"])
            ),
...
def new_world(
    ...
    *,
    ...
    return WorldState(
        ...
        #
        # `new_world` returns an empty world: no plants, no fixtures, no
        # animals, no collectibles. Stamping it here would declare an empty
        # world to be a whole composition -- and an earlier draft did exactly
        # that, so 0/0/0/0 characterized as a fresh composition. The stamps
        # belong at the END of successful starter generation, where there is a
        # population to describe; see `generate_initial_world` in generation.py.
    )
